...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/server_status_server_centos" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus Failed to download server service management script !" && exit 1
		fi
	...
	echo -e "${Info} ServerStatus The server service management script download is complete !"
}
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/server_status_client_centos" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus Client service management script download failed !" && exit 1
		fi
...
Install_caddy(){
	echo
	echo -e "${Info} Whether the script automatically configures the HTTP service (online monitoring website on the server side), if N is selected, please configure the root directory of the website in other HTTP services as：${Green_font_prefix}${web_file}${Font_color_suffix} [Y/n]"
...
echo && echo -e "  ServerStatus 一键安装管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
 ...
 ————————————
 ${Green_font_prefix} 1.${Font_color_suffix} install client
 ${Green_font_prefix} 2.${Font_color_suffix} update client
 ${Green_font_prefix} 3.${Font_color_suffix} Uninstall the client
————————————
 ${Green_font_prefix} 4.${Font_color_suffix} start client
 ${Green_font_prefix} 5.${Font_color_suffix} stop client
 ${Green_font_prefix} 6.${Font_color_suffix} restart client
————————————
 ${Green_font_prefix} 7.${Font_color_suffix} Set Client Configuration
 ${Green_font_prefix} 8.${Font_color_suffix} View client information
 ${Green_font_prefix} 9.${Font_color_suffix} View client logs
————————————
...
echo && echo -e "  ServerStatus 一键安装管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
 ...
 ${Green_font_prefix} 0.${Font_color_suffix} upgrade script
 ————————————
 ${Green_font_prefix} 1.${Font_color_suffix} Install server
 ${Green_font_prefix} 2.${Font_color_suffix} update server
 ${Green_font_prefix} 3.${Font_color_suffix} Uninstall the server
————————————
 ${Green_font_prefix} 4.${Font_color_suffix} Start the server
 ${Green_font_prefix} 5.${Font_color_suffix} stop server
 ${Green_font_prefix} 6.${Font_color_suffix} Restart the server
————————————
 ${Green_font_prefix} 7.${Font_color_suffix} Set server configuration
 ${Green_font_prefix} 8.${Font_color_suffix} View server information
 ${Green_font_prefix} 9.${Font_color_suffix} View server logs
————————————
 ${Green_font_prefix}10.${Font_color_suffix} Switch to client menu" && echo
if [[ -e "${server_file}/sergate" ]]; then
	check_pid_server
	if [[ ! -z "${PID}" ]]; then
		echo -e " Current status: server ${Green_font_prefix}Installed${Font_color_suffix} and ${Green_font_prefix}activated${Font_color_suffix}"
	else
		echo -e " Current status: server ${Green_font_prefix}Installed${Font_color_suffix} but ${Red_font_prefix}have not started${Font_color_suffix}"
	fi
else
